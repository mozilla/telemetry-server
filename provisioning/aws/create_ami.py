...
class AmiCreator:
    ...
    def create(self, description=None):
        self.launcher.go()
        conn = self.launcher.get_connection()
        instance = self.launcher.get_instance()
        ...
        base_name = self.launcher.config["name"]
        ami_name = "{0}-{1}".format(base_name, date.today().strftime("%Y%m%d"))
        ami_desc = description
        if ami_desc is None:
            ami_desc = self.launcher.config.get("description")
        if ami_desc is None:
            ami_desc = 'Generic Telemetry Image'
        ...
        self.launcher.terminate(conn, instance)
