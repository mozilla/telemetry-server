...
### Note: To run the am_exceptions export for a specific day, make a copy of the
...
LOG=~/cron_am_exceptions.log
cd ~/telemetry-server/
echo "Running am_exceptions export for $(date -d 'yesterday' +%Y%m%d)" >> $LOG
time /usr/bin/python -u -m provisioning.aws.launch_worker mapreduce/addons/am_exceptions_launch_config.json --instance-name irving-cron-am_exceptions-worker &>> $LOG
echo "Done" >> $LOG
