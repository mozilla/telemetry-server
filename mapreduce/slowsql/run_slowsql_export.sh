...
### Note: To run the slowsql export for a specific day, make a copy of the
...
LOG=~/cron_slowsql.log
cd ~/telemetry-server/
echo "Running slowsql export for $(date -d 'yesterday' +%Y%m%d)" >> $LOG
time /usr/bin/python -u -m provisioning.aws.launch_worker mapreduce/slowsql/slowsql_launch_config.json --instance-name mreid-cron-slowsql-worker &>> $LOG
echo "Done" >> $LOG
